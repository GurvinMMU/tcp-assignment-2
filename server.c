/*	server.c file
	...
	Members: 1) Gurminder Singh a/l Mahinder Singh	1121118711
		 2) Harvindran A/l Selvaraj  1102703337	 */
...
main(){
	...
	printf("\nWaiting for the connection to established............. [bind]\n");
	...
	if(fork() == 0){
	  close(sock);
	  strcpy(buffering,"***** Good Day, welcome to the our File Repository server.***** \n**\nPress\n1.Download the File\n2.upload the File\n3.Create the Directory\n4.Delete the Directory(including all the sub directory)**\n[type Exit to quit the server] : ");
	...
	{	
		...
		strcat(buffering,"[List of files in Server Directory]\nIn order to carry out the Download process pls select a filefrom the Server Directory:\n\n");
	...
	if(filepointer==NULL)
        {
            strcpy(buffering," There is no such file existing in the server.Please input in the correct file_name with its extension.");
	    send(nsock,  buffering, BUFFER_SIZE, 0);  
	    exist = false; 
